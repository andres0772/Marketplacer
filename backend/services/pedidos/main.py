...
def get_orders(estado: str | None = None, db: Session = Depends(get_db)):
...
async def get_order(id: int, db: Session = Depends(get_db)):
    ...
    # --- INICIO: Enriquecer ítems con nombre del producto ---
        ...
        for item in db_order.items:
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Añadimos dinámicamente el nombre al objeto item.
                    # Pydantic V2 lo manejará si el modelo lo permite.
                    item.nombre_producto = producto_data.get("nombre", "Producto no encontrado")
                else:
                    item.nombre_producto = "Producto no encontrado"
            except httpx.RequestError:
                item.nombre_producto = "Error al contactar servicio"
    # --- FIN: Enriquecer ítems ---
    return db_order # FastAPI se encargará de serializar el objeto enriquecido


@router.post("/", response_model=OrderRead)
async def create_order(order: OrderCreate, db: Session = Depends(get_db)):
    # --- INICIO: Validación de Seguridad ---
    # Nos aseguramos de que el ID de usuario sea válido antes de procesar.
    if not order.id_usuario or order.id_usuario <= 0:
        raise HTTPException(
            status_code=422, detail="Se requiere un ID de usuario válido para crear un pedido."
        )
...
async def update_order(id: int, order: OrderUpdate, db: Session = Depends(get_db)):
    ...
    # --- INICIO: Notificar al servicio de pagos si se cancela el pedido ---
    if order.estado == "cancelled":
        try:
            async with httpx.AsyncClient() as client:
                # Asumimos que el servicio de pagos tiene un endpoint para actualizar por ID de pedido
                # y que podemos cambiar su estado a 'cancelled' también.
                payment_update_payload = {"estado": "cancelled"}
                url = f"{settings.PAGOS_SERVICE_URL}/api/v1/pagos/by-order/{id}"
                await client.put(url, json=payment_update_payload)
        except (httpx.RequestError, httpx.HTTPStatusError) as e:
            # Es importante registrar esto, aunque no revertimos la cancelación del pedido.
            print(
                f"ADVERTENCIA: El pedido {id} se canceló, pero no se pudo actualizar el estado del pago asociado. Error: {e}"
            )
    # --- FIN: Notificación ---

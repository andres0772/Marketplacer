...
def get_pagos(estado: str | None = None, db: Session = Depends(get_db)):
    query = db.query(Payment)
    if estado:
        # Si se proporciona un estado, filtramos la consulta
...
def update_pago_by_order_id(
    ...
    # Actualizamos los campos que lleguen en el payload
    if pago_update.monto is not None:
        db_pago.monto = pago_update.monto
    if pago_update.estado is not None:
        db_pago.estado = pago_update.estado

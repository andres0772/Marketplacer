...
# Modelo para leer los datos del usuario (sin la contraseña)
...
async def register_user(user: UserRegister):
    ...
    # Asignación de ID más robusta para evitar duplicados en memoria
    last_id = users_db[-1]["id"] if users_db else 0
    user_dict["id"] = last_id + 1
    del user_dict["password"]  # No guardamos la contraseña en texto plano

    users_db.append(user_dict)

    # --- INICIO: Generar y devolver token después del registro ---
...
async def login(form_data: OAuth2PasswordRequestForm = Depends()):
    # El campo 'username' del formulario OAuth2 lo usaremos como el email
    user_email = form_data.username
    user = authenticate_user(user_email, form_data.password)
    if not user:
        raise HTTPException(
            status_code=400,
